    ...
    const fetchInitialData = async () => {
      try {
        ...
        try {
          ...
          const goalData = userGoalResponse.data || userGoalResponse;
          if (goalData && !goalData.null) {
            setUserGoal(goalData);
            if (goalData.nutritionalGoal) {
              setNutritionalGoal(goalData.nutritionalGoal);
            }
            if (goalData.exerciseLevel) {
              setExerciseLevel(goalData.exerciseLevel);
            }
          }
        } catch (err) {
  ...
  const handleShowDietPlans = async () => {
    try {
      ...
      try {
        ...
        // Handle both old and new response formats
        const mealsData = mealsResponse.data || mealsResponse;
        if (mealsData && Array.isArray(mealsData) && mealsData.length > 0) {
          setRecommendedMeals(mealsData);
        } else if (Array.isArray(mealsResponse) && mealsResponse.length > 0) {
          setRecommendedMeals(mealsResponse);
        } else {
