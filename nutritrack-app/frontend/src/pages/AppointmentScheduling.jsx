  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dieticiansData = dieticiansRes.data || dieticiansRes;
        const appointmentsData = appointmentsRes.data || appointmentsRes;
        
        setDieticians(dieticiansData.dieticians || dieticiansData.dieticiansList || []);
        setAppointments(appointmentsData.appointments || appointmentsData.appointmentsList || []);
        
        // Set default dietician if available
        const dieticiansList = dieticiansData.dieticians || dieticiansData.dieticiansList || [];
        if (dieticiansList.length > 0) {
          setDieticianId(dieticiansList[0].DieticianID);
        }
    ...
    try {
      ...
      if (response.success) {
        ...
        setAppointments(appointments.filter(app => {
          const appId = app.AppointmentID?.toString() || app.AppointmentID;
          const cancelId = appointmentId?.toString() || appointmentId;
          return appId !== cancelId;
        }));
      } else {
